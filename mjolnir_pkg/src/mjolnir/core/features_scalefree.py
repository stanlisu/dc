"""MIRROR of `agamotto/features_scalefree.py` — keep the two in lockstep.

mjolnir_pkg does not depend on agamotto_pkg, so the maths is duplicated
rather than imported. A parity test in agamotto_pkg/tests/ asserts the two
produce identical output whenever both are importable — change one, change
both. mjolnir builds obv/ad as raw cumulative talib.OBV/AD
(core/features.py:420-421) and so uses the DEFAULT obv_is_cumulative=True;
agamotto stores them already differenced and must pass False.
