...
_SWEEP_VOL_FILTERS = [
    ...
    # Trailing vol-quantile ENTRY gates (2026-08-16). `high_vol` (r028) is the
    # trailing-MEDIAN split; it was implemented but generated by nothing, so it
    # appeared in ZERO of the 59 sweep regimes and every deployed leg was
    # parented on VOLUME. Listed here as a LITERAL so obfuscation/
    # extract_inventory.py (_ATOM_SET_SUFFIXES `_FILTERS`) assigns each a regime
    # code — this list is their ONLY extraction site, since _VOL_QUANTILE_ATOMS
    # below carries an `_ATOMS` suffix the extractor does not scan and the
    # BASE_REGIMES entries are all `_and_` conjunctions, which it skips.
...
# Each trailing vol-quantile atom -> the cutoff column it compares
# `price_range_pct` against. The cutoffs are built per SYMBOL on the wide frame
# by research.engineer_features (VOL_QUANTILE_FEATURES / VOL_Q_LEVELS); the
# atom only reads them, which is why there is NO rolling fallback here.
_VOL_QUANTILE_ATOMS = {
...
# Scope B (2026-08-16): the 33 incumbent regimes, PLUS each of them additionally
# gated on one trailing vol-quantile cutoff — 3-atom conjunctions (2-atom for
...
def apply_filter_mask(
    ...
    # Trailing vol-quantile ENTRY gate. Resolved ONCE, above the position split,
    # because the predicate is position-INVARIANT: it says nothing about
    # direction, only whether the bar's range is large enough to clear the fixed
    # round-trip cost. (high_vol/low_vol below are the same predicate but are
    # duplicated verbatim into both branches — do not copy that.)
    #
    # BOTH columns go through _require_col and there is NO rolling fallback: the
    # `q50` ternary's else-branch below would recompute a rolling quantile on
    # whatever frame it is handed, and on the STACKED vertical panel that window
    # crosses symbol boundaries. It is dead today (q50 is always materialised);
    # it is not replicated here.
    if isinstance(filter_name, str) and filter_name in _VOL_QUANTILE_ATOMS:
        cutoff_col = _VOL_QUANTILE_ATOMS[filter_name]
