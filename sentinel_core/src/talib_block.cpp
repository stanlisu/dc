             ...
             std::vector<std::pair<std::string, std::vector<double>>>& out)
{
    ...
    one("rsi",    TA_RSI(0, n - 1, c, 14, &beg, &cnt, r.data()));
    one("rsi_7",  TA_RSI(0, n - 1, c, 7,  &beg, &cnt, r.data()));
    one("rsi_28", TA_RSI(0, n - 1, c, 28, &beg, &cnt, r.data()));
    ...
    {   // MACD: the reference keeps macd and macdhist (signal is discarded).
        ...
        if (rc != TA_SUCCESS) {
            out.emplace_back("macd", std::vector<double>(n, NA));
            out.emplace_back("macdhist", std::vector<double>(n, NA));
        } else {
            out.emplace_back("macd", place(n, beg, cnt, macd));
            out.emplace_back("macdhist", place(n, beg, cnt, hist));
        }
    }
    {   // STOCH defaults in the Python wrapper: 5, 3, SMA, 3, SMA
        ...
        if (rc != TA_SUCCESS) {
            out.emplace_back("stoch_k", std::vector<double>(n, NA));
            out.emplace_back("stoch_d", std::vector<double>(n, NA));
        } else {
            out.emplace_back("stoch_k", place(n, beg, cnt, k));
            out.emplace_back("stoch_d", place(n, beg, cnt, d));
        }
    }

    one("cci",      TA_CCI(0, n - 1, h, l, c, 14, &beg, &cnt, r.data()));
    one("adx",      TA_ADX(0, n - 1, h, l, c, 14, &beg, &cnt, r.data()));
    one("dx",       TA_DX(0, n - 1, h, l, c, 14, &beg, &cnt, r.data()));
    one("plus_di",  TA_PLUS_DI(0, n - 1, h, l, c, 14, &beg, &cnt, r.data()));
    one("minus_di", TA_MINUS_DI(0, n - 1, h, l, c, 14, &beg, &cnt, r.data()));
    one("mom",      TA_MOM(0, n - 1, c, 10, &beg, &cnt, r.data()));
    one("roc",      TA_ROC(0, n - 1, c, 10, &beg, &cnt, r.data()));
    one("willr",    TA_WILLR(0, n - 1, h, l, c, 14, &beg, &cnt, r.data()));
    one("cmo",      TA_CMO(0, n - 1, c, 14, &beg, &cnt, r.data()));
    one("atr",      TA_ATR(0, n - 1, h, l, c, 14, &beg, &cnt, r.data()));
    one("natr",     TA_NATR(0, n - 1, h, l, c, 14, &beg, &cnt, r.data()));
    ...
    {   // BBANDS defaults: 5, 2.0, 2.0, SMA. Middle band is discarded.
        ...
        if (rc != TA_SUCCESS) {
            out.emplace_back("bb_upper", std::vector<double>(n, NA));
            out.emplace_back("bb_lower", std::vector<double>(n, NA));
        } else {
            out.emplace_back("bb_upper", place(n, beg, cnt, up));
            out.emplace_back("bb_lower", place(n, beg, cnt, lo));
        }
    }

    one("sar", TA_SAR(0, n - 1, h, l, 0.02, 0.2, &beg, &cnt, r.data()));
    one("obv", TA_OBV(0, n - 1, c, v, &beg, &cnt, r.data()));
    one("ad",  TA_AD(0, n - 1, h, l, c, v, &beg, &cnt, r.data()));
    one("mfi", TA_MFI(0, n - 1, h, l, c, v, 14, &beg, &cnt, r.data()));
    one("std", TA_STDDEV(0, n - 1, c, 14, 1.0, &beg, &cnt, r.data()));
}
